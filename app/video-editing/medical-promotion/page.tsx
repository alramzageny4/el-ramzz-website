...
function VideoPlayerWithBuffering({ src }: { src: string }) {
  ...
  useEffect(() => {
    ...
    const preloadVideoChunk = async (startByte: number, endByte: number) => {
      ...
      try {
        ...
        if (response.ok && response.status === 206) {
          // Successfully fetched chunk, mark as buffered
          bufferedRangesRef.current.add(rangeKey)
          // The browser will use this cached data when needed
        }
      } catch (error) {
        // Ignore abort errors
    ...
    // Calculate byte range from time
    const getByteRangeForTime = async (targetTime: number, duration: number) => {
      if (!duration || !video.duration) return null
      
      // Estimate: assume linear distribution (not perfect but works for most videos)
      ...
      const estimatedSize = video.buffered.length > 0 ? 
        ...
        1000000 // Default estimate: 1MB
      
      const startByte = Math.floor(ratio * estimatedSize)
      const endByte = Math.min(startByte + 500000, estimatedSize) // 500KB chunks
    ...
    const handleProgress = async () => {
      if (video.buffered.length > 0 && video.duration) {
        ...
        // If buffered data is less than 15 seconds ahead, pre-load more
        if (bufferedEnd - currentTime < 15 && bufferedEnd < duration) {
          // Calculate target time (20 seconds ahead)
          const targetTime = Math.min(currentTime + 20, duration - 1)
          
          // Pre-load chunks using Fetch API
          const byteRange = await getByteRangeForTime(targetTime, duration)
          if (byteRange) {
            preloadVideoChunk(byteRange.startByte, byteRange.endByte)
          }
          
          // No seeking - just pre-load chunks to avoid pause/play issues
        }
      }
    }

    const handleTimeUpdate = () => {
      // Continuously monitor and ensure buffering ahead
      if (video.buffered.length > 0 && video.duration) {
        const bufferedEnd = video.buffered.end(video.buffered.length - 1)
        const currentTime = video.currentTime
        
        // If we're getting close to the end of buffered data, force more buffering
    ...
    const handleLoadedMetadata = async () => {
      // Start aggressive pre-buffering immediately after metadata loads
      if (video.duration) {
        // Pre-load first 30 seconds
...
export default function MedicalPromotionPage() {
  const { t } = useLanguage()
  ...
  // Videos data - فيديوهات الترويج الطبي
  const videos: Array<{
    id: number
    title: string
    videoSrc: string
    ...
    {
      id: 1,
      title: 'ترويج طبي احترافي - عرض الخدمات الطبية',
      videoSrc: 'https://video-cdn-elramz.b-cdn.net/doctor-20251127T104520Z-1-001/doctor/1.mp4',
      thumbnail: '',
    },
    {
      id: 2,
      title: 'كارت ترويجي للأطباء - التخصصات الطبية',
      videoSrc: 'https://video-cdn-elramz.b-cdn.net/doctor-20251127T104520Z-1-001/doctor/2.mp4',
      thumbnail: '',
    },
    {
      id: 3,
      title: 'فيديو دعائي طبي - العيادات والمستشفيات',
      videoSrc: 'https://video-cdn-elramz.b-cdn.net/doctor-20251127T104520Z-1-001/doctor/3.mp4',
      thumbnail: '',
    },
    {
      id: 4,
      title: 'مونتاج ترويج طبي - الخبرة والتميز',
      videoSrc: 'https://video-cdn-elramz.b-cdn.net/doctor-20251127T104520Z-1-001/doctor/4.mp4',
      thumbnail: '',
    },
  ]
